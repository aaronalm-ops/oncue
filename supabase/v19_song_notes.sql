-- V19: song-level personal notes.
--
-- The gap: user_notes was keyed (user_id, section_id, instrument), and a song
-- created from a setlist has NO sections until a chart or song memory builds
-- them — apply_song_memory no-ops for a song nobody has played before. So the
-- one moment you most want to jot something down (first rehearsal of a new
-- song) is the one moment there is nowhere to put it.
--
-- Rejected alternative: auto-create a synthetic "Song" section. It would work
-- today and cost nothing, but capture_service_memory snapshots sections into
-- worship_leader_song_memory and library_song_arrangement, so the fake section
-- would propagate into every future setlist of that song, permanently.
--
-- So: user_notes rows now target EITHER a section or a song, never both.

ALTER TABLE user_notes ALTER COLUMN section_id DROP NOT NULL;
ALTER TABLE user_notes ADD COLUMN IF NOT EXISTS song_id uuid
  REFERENCES songs ON DELETE CASCADE;

-- Drop whatever name the original UNIQUE (user_id, section_id, instrument) got.
-- NULLs are distinct in a unique index, so leaving it would let a user pile up
-- unlimited song-level notes for the same song.
DO $do$
DECLARE c text;
BEGIN
  FOR c IN
    ...
    WHERE rel.relname = 'user_notes' AND con.contype = 'u'
  LOOP
    EXECUTE format('ALTER TABLE user_notes DROP CONSTRAINT %I', c);
  END LOOP;
END
$do$;

ALTER TABLE user_notes DROP CONSTRAINT IF EXISTS user_notes_target_chk;
ALTER TABLE user_notes ADD CONSTRAINT user_notes_target_chk
  CHECK ((section_id IS NULL) <> (song_id IS NULL));

-- PLAIN unique constraints, deliberately NOT partial indexes. Two reasons:
--   1. NULLs are distinct by default, which is exactly the semantics we want.
--      Every song note has section_id NULL, so they never collide on the
--      section constraint; every section note has song_id NULL, so they never
--      collide on the song one. Each constraint only ever polices its own kind.
--   2. supabase-js .upsert({ onConflict: 'user_id,section_id,instrument' })
--      emits ON CONFLICT (cols) with no WHERE clause, and Postgres CANNOT
--      infer a partial index without its predicate. Partial indexes here would
--      break saveNote() for every existing section note with
--      "there is no unique or exclusion constraint matching the ON CONFLICT
--      specification" — a total regression on a feature that works today.
ALTER TABLE user_notes DROP CONSTRAINT IF EXISTS user_notes_section_uniq;
ALTER TABLE user_notes ADD CONSTRAINT user_notes_section_uniq
  UNIQUE (user_id, section_id, instrument);
ALTER TABLE user_notes DROP CONSTRAINT IF EXISTS user_notes_song_uniq;
ALTER TABLE user_notes ADD CONSTRAINT user_notes_song_uniq
  UNIQUE (user_id, song_id, instrument);
CREATE INDEX IF NOT EXISTS idx_user_notes_song
  ON user_notes (song_id) WHERE song_id IS NOT NULL;

-- RLS is unchanged: the existing FOR ALL policy is own-row on user_id, which
-- covers song-scoped rows exactly as it covered section-scoped ones.

-- ============================================================
-- ingest_chart, re-emitted from v16 with ONE change: the ghost sweep now
-- also looks for song-level notes. Everything else is byte-identical to v16.
-- ============================================================
...
BEGIN
  ...
  WHERE s.id = e.id
    ...
    -- v19: song-level notes hang off songs.id with no section to join
    -- through. Without this clause a song whose ONLY note is a song note
    -- looks empty to the ghost sweep and gets deleted on the next chart
    -- upload, taking the note with it.
    AND NOT EXISTS (
      SELECT 1 FROM user_notes un WHERE un.song_id = s.id
    );
