...
-- Supabase installs extensions into the `extensions` schema, so the trigram
-- functions are extensions.word_similarity(), not public.* — the RPC's
-- search_path below includes both. (Locally pg_trgm lands in public; same
-- search_path covers that.)
